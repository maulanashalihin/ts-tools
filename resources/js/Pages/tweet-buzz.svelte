...
function saveTweet()
{
  ...
  axios.post("/tweet-buzz",{tweet_id, campaign_id : campaign.id}).then(response=>{
   buzzes = [response.data,...buzzes]
   tweet_id = ""
  })
    
}

  let pathname = "tweet-buzz";

  
  function deleteItem(item)
  {
    buzzes = buzzes.filter(i=>i.id!=item.id)
    axios.delete("/tweet-buzz/"+item.id)
  }
...
<Layouts>
    ...
    <div class="container lg:mt-6 max-w-lg mx-auto bg-white p-4 lg:p-8">
        ...
        <div class="grid gap-4 mb-3 mt-6"> 
            <p>Pilih Tweet untuk dibuzz dengan like dan retweet</p> 
        ...
        <form on:submit|preventDefault={saveTweet}    class="space-y-6">
            ...
            <div class="space-y-1">
             ...
             <input required bind:value="{tweet_id}" class="w-full block border border-gray-200 rounded px-3 py-2 leading-6 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" type="text" id="tweet_id" placeholder="1587098878763233286" />
            </div>
