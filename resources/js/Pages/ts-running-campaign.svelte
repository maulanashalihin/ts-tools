<script>
  ...
  if (Date.now() < campaign.end_time && campaign.status == "running") {
    setInterval(() => {
      ...
      campaign.hours = Math.floor(
        (distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
      );
      campaign.minutes = Math.floor(
  ...
  if (
    attendance.next_round_time &&
    dayjs().isAfter(attendance.next_round_time)
  ) {
  ...
  function saveAkadTS() {
    attendance.next_round_time = dayjs()
      .add(attendance.next_round_interval, "minute")
      .valueOf();
    attendance.reminder_round_time = dayjs()
      .add(attendance.next_round_interval, "minute")
      .valueOf();
...
<div>
  <TsLayouts>
      ...
      <div class="grid gap-4 mb-3">
            ...
            >
              <div
                ...
                aria-valuenow={Math.floor(
                  (attendance.current_round / attendance.round_number) * 100
                )}
                aria-valuemin="0"
                aria-valuemax="100"
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          {#if attendance.next_round_time}
            <div>
              <label class="font-medium" for="tk-form-elements-name"
                >Round Selanjutnya</label
              >
              <div>
        ...
        <div>
          <h3 class="font-semibold">Campaign berakhir dalam...</h3>
    ...
    <div class="container my-3 grid gap-3  lg:my-6 max-w-lg mx-auto ">
      <div class="font-bold text-lg">1. Tweet Konten ini</div>
      {#each tweets as item}
        <!-- content here -->
          ...
          <div class="  mt-3 flex gap-3">
            {#if !tweet_done.includes(item.id)}
              <a
                on:click={() => {
                  UpdateScore(true);
                  tweet_done = [...tweet_done, item.id];
                }}
                href="https://twitter.com/intent/tweet?text={encodeURIComponent(
                  item.content + hashtag_text + '\n' + generateUniqCode()
                )}"
                rel="noreferrer"
                target="_blank"
                class="  py-1  inline-flex"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
          ...
          <div class="  mt-3 flex gap-3">
            ...
            {#if !retweet_done.includes(item.id)}
              <a
                on:click={() => {
                  UpdateScore();
                  retweet_done = [...retweet_done, item.id];
                }}
    ...
    <div class="container my-3 grid gap-3  lg:my-6 max-w-lg mx-auto ">
      <div class="font-bold  text-lg">3. Komentar pada tweet ini</div>
          ...
          <div class="  mt-3">
            {#if !reply_done.includes(item.id)}
              <a
                on:click={() => {
                  UpdateScore(true);
                  reply_done = [...reply_done, item.id];
                }}
                href="https://twitter.com/intent/tweet?in_reply_to={item.tweet_id}&text={encodeURIComponent(
                  pickTweet() + hashtag_text + '\n' + generateUniqCode()
                )}"
                rel="noreferrer"
                target="_blank"
                class="  py-1  inline-flex"
    ...
    <div class="container my-3 grid gap-3  lg:my-6 max-w-lg mx-auto ">
      <div class="font-bold  text-lg">4. Follow Akun Twitter ini</div>
  ...
  <div class="grid grid-cols-2 p-6 gap-3">
    {#each medias as item}
      ...
      <a
        on:click={() => {
          UpdateScore(true);
          tweet_done = [...tweet_done, active_tweet.id];
          mediaModal = false;
        }}
        href="https://twitter.com/intent/tweet?text={encodeURIComponent(
          active_tweet.content + hashtag_text + '\n' + generateUniqCode()
        )}+{item.twitter_url}"
        rel="noreferrer"
        target="_blank"
        class="  py-1  inline-flex"
  ...
  <div class="p-6">
    ...
    <form on:submit|preventDefault={saveAkadTS} class="space-y-3 mt-3">
      ...
      <div class="space-y-1">
        ...
        <input
          required
          bind:value={attendance.round_number}
          class="w-full block border border-gray-200 rounded px-3 py-2 leading-6 focus:border-amber-500 focus:ring focus:ring-amber-500 focus:ring-opacity-50"
          type="number"
          id="round_number"
          placeholder="3"
        />
      ...
      <div class="space-y-1">
        <label class="font-medium" for="next_round_interval"
          >Jeda Antar Round</label
        >
        <select
          required
          bind:value={attendance.next_round_interval}
      ...
      <div class="space-y-1">
        ...
        <div class="relative">
          ...
          <input
            bind:value={attendance.troop_username}
        ...
        <small
          >pastikan username twitter ini benar (http://twitter.com/{attendance.troop_username})</small
        >
      </div>
      <button
