...
`
      ...
      function saveProject()
      {
       ...
       if(Project.id)
       {
        Inertia.put("/strat-plan/"+Project.id,Project)
       }else
       {
        Inertia.post("/strat-plan",Project)
       }
      }
  ...
  function deleteProject()
  {
    ...
    if(check)
    {
      Inertia.delete("/strat-plan/"+Project.id)
    }
  }
                  ...
                  <div class="flex flex-col rounded shadow-sm bg-white overflow-hidden md:w-2/3">
                    ...
                    <div class="p-5 lg:p-6 grow w-full">
                      <form on:submit|preventDefault={saveProject}    class="space-y-6">
                        ...
                        <div class="space-y-1">
                          <label for="title" class="font-medium">{t("Judul Project")}</label>
                          <input bind:value={Project.title} class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" type="text" id="title" placeholder="Kontra Isu Anti Khilafah" required />
                        </div>
                        <div class="space-y-1">
                          <label class="font-medium" for="description">{t("Deskripsi Project")}</label>
                          <div id="description">
                        ...
                        <div class="space-y-1">
                          <label  for="start_time_string" class="font-medium">Waktu Project Mulai</label>
                          <input bind:value={Project.start_time_string} class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" type="date" id="start_time_string"  required/>
                          <small class="text-gray-400">waktu dalam {timezone}</small>
                        </div>
                        <div class="space-y-1">
                          <label  for="end_time_string" class="font-medium">Waktu Project Berakhir</label>
                          <input bind:value={Project.end_time_string} class="block border border-gray-200 rounded px-3 py-2 leading-6 w-full focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" type="date" id="end_time_string"   required/>
                          <small class="text-gray-400">waktu dalam {timezone}</small>
                        </div>
                    ...
                    <div class="flex flex-col rounded shadow-sm bg-white overflow-hidden">
                      ...
                      <div class="p-5 lg:p-6 grow w-full border-l-4 border-red-300">
                        ...
                        <p class="text-gray-500 mb-4">
                          Are you sure you would like to delete this Project? Please be careful since this action cannot be undone!
                        </p>
