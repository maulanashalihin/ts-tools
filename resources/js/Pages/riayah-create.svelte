...
{  
   if(campaign.buttons.length)
   {
    ...
    if(campaign.id)
    {
        Inertia.put("/riayah/"+campaign.id,campaign)
    }else{
        Inertia.post("/riayah",campaign)
    }
   }else{
    ...
    <div class="container grid gap-3  p-4 lg:p-8  max-w-lg mx-auto ">
        ...
        <form class=" space-y-3" on:submit|preventDefault={saveMessage}>
            ...
            <div class="space-y-1">
                ...
                <textarea   bind:value="{campaign.text}" class="w-full block border border-gray-200 rounded px-3 py-2 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" id="text" rows="8" placeholder="Enter further details"></textarea>
                ...
                <div class=" grid gap-1 my-2">
                    ...
                    {#each campaign.buttons as button}
                         ...
                         <div>
                            ...
                            <div class="font-semibold inline-flex px-2 py-1 leading-4 campaigns-center space-x-1 text-sm rounded text-teal-700 bg-teal-200">
                              <span>{button}</span> 
                              <button type="button" on:click="{()=>{campaign.buttons = campaign.buttons.filter(btn=>btn != button)}}" class="focus:outline-none text-teal-600 hover:text-teal-400 focus:ring focus:ring-teal-500 focus:ring-opacity-50 active:text-teal-600">
